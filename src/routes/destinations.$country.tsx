...
export const Route = createFileRoute("/destinations/$country")({
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-3xl px-6 py-32 text-center">
      ...
      <Link to="/" className="mt-8 inline-block text-sm font-semibold" style={{ color: "var(--brand-red)" }}>
        ← Retour à l'accueil
      </Link>
    </div>
  ),
  errorComponent: ({ reset }) => (
    <div className="mx-auto max-w-3xl px-6 py-32 text-center">
      ...
      <button onClick={reset} className="mt-6 rounded-full px-5 py-3 text-sm font-semibold text-white" style={{ backgroundColor: "var(--brand-red)" }}>Réessayer</button>
    </div>
  ),
  ...
  return (
    <div>
      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${d.accent}cc 0%, rgba(0,0,0,0.55) 100%)` }} />
        <div className="relative mx-auto max-w-7xl px-6 py-28 md:py-36 text-white">
          ...
          <h1 className="mt-5 max-w-3xl text-5xl font-semibold leading-[1.05] md:text-7xl" style={{ fontFamily: "var(--font-display)" }}>
            {d.name}
          </h1>
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <Link to="/request" search={{ service: d.slug }} className="inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white" style={{ backgroundColor: "var(--brand-red)" }}>
              Démarrer ma demande <ArrowRight className="h-4 w-4" />
            </Link>
            <Link to="/contact" className="inline-flex items-center gap-2 rounded-full border border-white/40 bg-white/10 px-6 py-3 text-sm font-semibold text-white backdrop-blur hover:bg-white/20">
              Parler à un expert
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: d.accent }}>Pourquoi {d.name}</p>
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {d.highlights.map((h) => (
            <div key={h.title} className="rounded-3xl border border-border bg-card p-8">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl" style={{ backgroundColor: d.accent, color: "white" }}>
      ...
      <section className="py-20" style={{ backgroundColor: "var(--brand-cream)" }}>
        <div className="mx-auto max-w-7xl px-6">
          <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: d.accent }}>Villes phares</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <div className="grid gap-12 lg:grid-cols-2">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: d.accent }}>Visa & démarches</p>
            <h2 className="mt-3 text-4xl font-semibold md:text-5xl">Le bon visa pour votre projet.</h2>
            ...
            <div className="mt-6 flex items-start gap-3 rounded-2xl p-5" style={{ backgroundColor: `${d.accent}14` }}>
          ...
          <div className="rounded-3xl border border-border bg-card p-8">
            ...
            <p className="mt-6 text-xs text-muted-foreground">Liste indicative — variable selon votre profil et le consulat.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20" style={{ backgroundColor: "var(--brand-navy)", color: "white" }}>
        <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-2">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: "#ffb3bd" }}>Infos pratiques</p>
            <h2 className="mt-3 text-4xl font-semibold md:text-5xl">{d.name} en chiffres.</h2>
            <div className="mt-10 grid grid-cols-2 gap-4">
              {d.practical.map((p) => (
                <div key={p.label} className="rounded-2xl border border-white/15 bg-white/5 p-5 backdrop-blur">
          ...
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: "#ffb3bd" }}>Nos conseils</p>
            <h2 className="mt-3 text-4xl font-semibold md:text-5xl">À ne pas manquer.</h2>
            <ul className="mt-10 space-y-4">
              {d.tips.map((tip) => (
                <li key={tip} className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="grid gap-8 md:grid-cols-2 md:items-center">
            <div>
              ...
              <p className="mt-4 text-lg text-muted-foreground">
                Nos services dédiés à votre projet : choisissez celui qui correspond et lancez votre demande en quelques minutes.
              </p>
            </div>
            <div className="flex flex-col gap-3">
              {d.serviceFocus.map((s) => (
                <Link key={s.to + s.label} to={s.to} className="inline-flex items-center justify-between gap-2 rounded-full bg-white px-6 py-4 text-sm font-semibold text-foreground shadow-sm transition-all hover:-translate-y-0.5">
                  {s.label} <ArrowRight className="h-4 w-4" style={{ color: d.accent }} />
                </Link>
              ))}
              <Link to="/request" search={{ service: d.slug }} className="inline-flex items-center justify-center gap-2 rounded-full px-6 py-4 text-sm font-semibold text-white" style={{ backgroundColor: "var(--brand-red)" }}>
                Démarrer ma demande <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
          </div>
