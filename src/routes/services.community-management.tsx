...
export const Route = createFileRoute("/services/community-management")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Gestion de réseaux sociaux, SEO, sites web et identité visuelle : votre communauté, votre image, votre impact." },
      { property: "og:title", content: "Community Management & Marketing Digital — VoyageonsEnsemble" },
  ...
  return (
    <>
      <ServiceDetail
        ...
        steps={[
          { title: "Audit & stratégie", desc: "Analyse de votre marque, de vos cibles et de vos concurrents." },
          { title: "Production", desc: "Calendrier éditorial, visuels et contenus prêts à publier." },
          { title: "Diffusion & ads", desc: "Publication, modération et campagnes publicitaires ciblées." },
          { title: "Mesure", desc: "Rapports mensuels, ajustements et recommandations d'amélioration." },
        ]}
        included={[
          { icon: Megaphone, t: "Réseaux sociaux" },
          { icon: Search, t: "SEO & Ads" },
          { icon: Globe2, t: "Sites web" },
          { icon: PenTool, t: "Identité visuelle" },
          { icon: BarChart3, t: "Reporting" },
        ]}
        faqs={[
          { q: "Sur quels réseaux intervenez-vous ?", a: "Facebook, Instagram, LinkedIn, TikTok, Twitter/X et YouTube selon votre audience cible." },
          { q: "Combien de temps avant de voir des résultats SEO ?", a: "Les premiers gains apparaissent en 6 à 12 semaines, avec une montée en puissance continue sur 3 à 6 mois." },
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24">
        <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: "var(--brand-red)" }}>Packs & tarifs</p>
        <h2 className="mt-3 text-3xl font-semibold md:text-4xl">Nos offres de Community Management & Digital Branding</h2>
        <p className="mt-3 max-w-2xl text-muted-foreground">Trois formules pensées pour accompagner votre marque, du démarrage à la stratégie premium.</p>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {packs.map((p) => (
            <article
              key={p.name}
              className="flex flex-col rounded-3xl border bg-card p-8"
              style={p.featured ? { borderColor: "var(--brand-red)", boxShadow: "0 10px 30px -15px rgba(229,49,69,0.35)" } : undefined}
            >
              {p.featured && (
                <p className="mb-3 text-xs font-semibold uppercase tracking-widest" style={{ color: "var(--brand-red)" }}>Le plus choisi</p>
              )}
