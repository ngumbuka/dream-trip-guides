...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contactez-nous pour démarrer votre projet de mobilité internationale." },
    ...
    <div>
      <section className="border-b border-border" style={{ backgroundColor: "var(--brand-cream)" }}>
        <div className="mx-auto max-w-5xl px-6 py-24 text-center">
          <p className="text-sm font-medium uppercase tracking-[0.2em]" style={{ color: "var(--brand-red)" }}>Contact</p>
          <h1 className="mt-4 text-5xl font-semibold leading-tight md:text-6xl">Parlons de votre projet.</h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
            Que votre projet soit clair ou encore en réflexion, nous sommes là pour en discuter. Réponse sous 24h.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-20 md:grid-cols-[1fr_1.4fr]">
        <aside className="space-y-6">
          {[
            ...
            <div key={i.label} className="flex items-start gap-4 rounded-2xl border border-border bg-card p-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl" style={{ backgroundColor: "var(--brand-navy)", color: "white" }}>
        ...
        >
          {sent ? (
            <div className="py-16 text-center">
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full" style={{ backgroundColor: "var(--brand-red)", color: "white" }}>
            ...
            <>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Type de projet</label>
              ...
              <div className="mt-4">
                <label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Votre message</label>
                <textarea rows={5} required className="mt-2 w-full rounded-xl border border-border bg-white px-4 py-3 text-sm outline-none focus:border-foreground" placeholder="Parlez-nous de votre projet…" />
              </div>
              <button
                type="submit"
...
function Field({ label, ...rest }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      ...
      <input {...rest} className="mt-2 w-full rounded-xl border border-border bg-white px-4 py-3 text-sm outline-none focus:border-foreground" />
    </div>
  );
}