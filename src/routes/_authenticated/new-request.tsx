...
function NewRequestPage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const { id } = await createFn({ data: {
        service_slug: slug, service_label: label,
      ...
      for (const f of files) {
        const signed = await signFn({ data: { request_id: id, file_name: f.name, mime_type: f.type, size_bytes: f.size } });
        const { error } = await supabase.storage.from("request-documents").uploadToSignedUrl(signed.path, signed.token, f);
        if (error) throw error;
        await confirmFn({ data: { request_id: id, storage_path: signed.path, file_name: f.name, mime_type: f.type, size_bytes: f.size } });
      }
      return id;
    },
    onSuccess: (id) => {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
      ...
      <p className="mt-3 text-muted-foreground">Décrivez votre projet — notre équipe vous répond sous 24h via la messagerie de votre espace.</p>

      <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }} className="mt-10 space-y-6 rounded-3xl border border-border bg-card p-8">
        <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input value={destination} onChange={(e) => setDestination(e.target.value)} placeholder="France, Canada, Cameroun…" className={inp} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min={1} max={50} value={travelers} onChange={(e) => setTravelers(Number(e.target.value))} className={inp} />
          </div>
          <div>
            <Label>Budget approximatif</Label>
            <input value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="ex. 2 000 — 4 000 €" className={inp} />
          </div>
        </div>
        <div>
          <Label>Votre projet en détail</Label>
          <textarea required rows={5} value={message} onChange={(e) => setMessage(e.target.value)} className={inp} placeholder="Parlez-nous de votre situation, vos objectifs, votre calendrier…" />
        </div>
        <div>
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {files.map((f, i) => (
                <li key={i} className="flex items-center justify-between rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <button type="button" onClick={() => setFiles((p) => p.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
                </li>
              ))}
            </ul>
          )}
