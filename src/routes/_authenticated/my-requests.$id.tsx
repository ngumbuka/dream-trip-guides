...
import { getMyRequest, sendMessage, signDocumentDownload, signDocumentUpload, confirmDocument } from "@/lib/requests.functions";
...
function RequestDetail() {
  ...
  const sendMut = useMutation({
    ...
    onSuccess: () => { setBody(""); qc.invalidateQueries({ queryKey: ["my-request", id] }); },
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const s = await signFn({ data: { request_id: id, file_name: f.name, mime_type: f.type, size_bytes: f.size } });
      const { error } = await supabase.storage.from("request-documents").uploadToSignedUrl(s.path, s.token, f);
      if (error) throw error;
      await confirmFn({ data: { request_id: id, storage_path: s.path, file_name: f.name, mime_type: f.type, size_bytes: f.size } });
      toast.success("Fichier ajouté"); qc.invalidateQueries({ queryKey: ["my-request", id] });
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      <Link to="/my-requests" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Retour</Link>
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold">{r.service_label}</h1>
          <p className="mt-1 text-sm text-muted-foreground">Créée le {new Date(r.created_at).toLocaleString()}</p>
        </div>
          ...
          <ul className="mt-4 space-y-2">
            {documents.map((d: any) => (
              <li key={d.id} className="flex items-center justify-between rounded-lg border border-border bg-card px-3 py-2 text-sm">
                ...
                <button onClick={() => download(d.id)} className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground"><Download className="h-4 w-4" /> Télécharger</button>
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 space-y-3 rounded-3xl border border-border bg-card p-6">
          ...
          {messages.map((m: any) => (
            <div key={m.id} className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${m.author_role === "admin" ? "bg-muted" : "ml-auto text-white"}`}
              style={m.author_role !== "admin" ? { backgroundColor: "var(--brand-red)" } : undefined}>
              <p>{m.body}</p>
              <p className="mt-1 text-[10px] opacity-70">{m.author_role === "admin" ? "Équipe" : "Vous"} · {new Date(m.created_at).toLocaleString()}</p>
            </div>
          ))}
          <form onSubmit={(e) => { e.preventDefault(); if (body.trim()) sendMut.mutate(); }} className="mt-4 flex gap-2">
            <input value={body} onChange={(e) => setBody(e.target.value)} placeholder="Votre message…" className="flex-1 rounded-full border border-border bg-white px-4 py-2 text-sm outline-none focus:border-foreground" />
