...
export function ServiceHero({
  ...
  ServiceDetailProps,
  "eyebrow" | "title" | "intro" | "image" | "imageAlt" | "ctaLabel" | "serviceSlug"
>) {
  ...
  return (
    <section className="relative isolate overflow-hidden min-h-[500px]">
      <img
        src={image}
        alt={imageAlt}
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-32 text-white">
        ...
        <h1 className="mt-3 max-w-3xl text-5xl font-semibold leading-[1.05] md:text-6xl animate-in fade-in slide-in-from-bottom-4 duration-700 delay-[400ms] fill-mode-both">
          {title}
        </h1>
        <p className="mt-6 max-w-2xl text-lg text-white/85 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-[600ms] fill-mode-both">
          {intro}
        </p>
        <div className="mt-8 flex flex-wrap gap-3 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-[800ms] fill-mode-both">
          <Link
            to="/request"
          ...
          <ol className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {steps.map((s, i) => (
              <li key={s.title} className="rounded-3xl border border-border bg-white p-6">
                <div className="text-sm font-semibold text-brand-red">Étape {i + 1}</div>
          ...
          <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {included.map((e) =>
              e.to ? (
                ...
                >
                  <e.icon className="mx-auto mb-3 h-8 w-8 opacity-80 text-brand-red" />
...
export function ServiceCta({
  ctaLabel = "Démarrer mon projet",
  serviceSlug,
}: Pick<ServiceDetailProps, "ctaLabel" | "serviceSlug">) {
  return (
    <Reveal>
